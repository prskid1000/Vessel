...
# xwin, which fetches Microsoft's CRT and Windows SDK for the one component that
# must be built for Microsoft's ABI rather than MinGW's: the Steam client
# emulator (build/gbe.sh), whose DLLs a game's MSVC-built steam_api64.dll calls
# through C++ vtables.
#
# **Only the tool is in the image, not what it downloads.** Microsoft's license
# lets anyone download and use the SDK but not redistribute it, and this image
# is published. build/gbe.sh runs xwin on first use -- accepting that license
# for whoever runs the build -- into the /work volume, at the SDK and CRT
# versions pinned in native/pins.env, and reuses it after that.
...
RUN set -eux; \
    ...
    install -m 0755 "/tmp/xwin-${XWIN_VERSION}-x86_64-unknown-linux-musl/xwin" /usr/local/bin/xwin; \
    rm -rf /tmp/xwin.tar.gz "/tmp/xwin-${XWIN_VERSION}-x86_64-unknown-linux-musl"; \
    xwin --version
