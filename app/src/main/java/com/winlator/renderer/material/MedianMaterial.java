 ...
 * patch of sky, and the matcher reports the wrong answer with exactly the same
 * confidence as a right one. Warping along a field containing those mistakes
 * tears a hole in an otherwise correct picture, and because the field is sampled
 * bilinearly the damage spreads into the neighbours that were right.
 *
 * <p>The standard answer, and the one FidelityFX runs as an explicit pass between
 * its search and its upscale, is a 3x3 <em>vector</em> median. Not a per-channel
 * median, which can invent a vector no block voted for by taking x from one
 * candidate and y from another: for each of the nine candidates the summed
 * distance to all nine is computed, and the one collectively closest to the rest
 * wins. The winner is therefore always a vector some block actually reported, and
 * a single outlier can never be it.
 *
 ...
 * portability gamble for no gain -- it unrolls to nine taps and eighty-one
 * distances, which is nothing at one texel per 8x8 block.
 *
 * <p>Sampling relies on the field being addressed at texel centres: with offsets
 * of exactly one texel a linear sampler returns the neighbour's value unblended,
 * so this needs no separate sampler state.
 */
public class MedianMaterial extends ScreenMaterial {
    ...
    protected String getFragmentShader() {
        return String.join("\n",
            ...
            // included against itself, contributing zero, which keeps the
