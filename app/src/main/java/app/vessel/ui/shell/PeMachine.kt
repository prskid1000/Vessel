 ...
 *
 * **This belongs in `core/`, and it is here because this pass does not own
 * `core/`.** `core/PeArchitecture.kt` already declares the enum and the machine
 * words; all that is missing upstream is the four reads below. It is requested in
 * `out/ui-needs-from-core.md` as `core/PeReader.kt`; moving it is a package
 * change and nothing else, which is why it is written as a pure function over a
 * [File] with no Compose and no Android in it.
 *
 ...
 * there: the two are the same colour and the same statement on every surface in
 * this product — *runs without translation* — so the extra reads would buy a
 * distinction nothing shows. [PeArchitecture.ARM64] is returned for both.
 *
 ...
 * file the app cannot open are all "we could not tell", which is exactly what the
...
object PeMachine {
    ...
    fun of(file: File): PeArchitecture {
