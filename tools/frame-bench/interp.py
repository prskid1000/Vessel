...
the alternatives on identical input. The defaults are what ships.

    newer_side   the newer-side round trip. On, as shipped: occside.py
                 measured it worth 4 levels on covered background and nothing
                 elsewhere, against the argument that it should fire on every
                 field gradient.
    drop         which source the older-side disagreement removes: "older" is
                 what ships, "newer" is what the geometry of two N-1-indexed
                 fields argues for, "none" switches the test off.
    border_gate  the in-frame fades on the source weights and on fitMoving.
                 Off reproduces the fades applied only to the occlusion term.
    photometry   "colour" scores the RGB targets, "luma" the R8 pair.
